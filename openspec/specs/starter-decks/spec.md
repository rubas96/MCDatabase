...
Show each hero's official preconstructed ("starter") deck on its detail view so the player
sees the full out-of-the-box deck — including the Core-Set basic cards that belong to it but
are not printed in the hero's own pack. The hero→decklist mapping is sourced from verified
public MarvelCDB decklists and embedded for offline use.
...
- **WHEN** the maintainer regenerates the data (`_fetch_starters.mjs`, or an equivalent that
